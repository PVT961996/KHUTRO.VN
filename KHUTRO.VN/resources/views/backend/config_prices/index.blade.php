...
$('#items').submit();}")) }}
                ...
                <div class="btn-group">
                    ...
                    <ul class="dropdown-menu pull-right">
                        <li>
                            <form id="form_export_to_excel" style="display: inline" action="{{ route('admin.configPrices.export') }}" method="POST">
                                {{ csrf_field() }}
                        ...
                        <li>
                            <form id="form_export_to_csv" style="display: inline" action="{{ route('admin.configPrices.export') }}" method="POST">
                                {{ csrf_field() }}
                ...
                <a class="btn btn-icon-only btn-default reload" href="{!! route('admin.configPrices.index') !!}" data-original-title="Reload" title="Reload"> <i class="fa fa-repeat"></i></a>
                <a class="btn btn-icon-only btn-default fullscreen" href="javascript:;" data-original-title="" title=""> </a>
                {!! Form::open(['id' =>'items', 'route' => ['admin.configPrices.destroy', 'MULTI'], 'method' => 'delete']) !!}
                {!! Form::close() !!}
            </div>
        </div>
        ...
        <div class="box box-primary">
            <div class="box-header">
                {!! Form::open(['method' => 'GET','route' => 'admin.configPrices.index','role' => 'search']) !!}
            ...
            <div class="box-body">
                <div class="table-scrollable">
                    @include('backend.config_prices.table')
                </div>
            </div>
            @if($configPrices->hasPages())
                <div class="box-footer">
                    {!! $configPrices->appends(['search' => Request::get('search')])->render() !!}
                </div>
            @endif
        </div>
    </div>
